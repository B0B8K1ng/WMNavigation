...
class Env:
    ...
    def run_experiment(self):
        ...
        elif self.cfg['part'] == 'down':
            instance_size = math.ceil(500 / self.cfg['instances'])  # 1000
            end_ndx = 1000
            start_ndx = 500 + self.cfg['instance'] * instance_size
        elif self.cfg['part'] == 'up':
            instance_size = math.ceil(500 / self.cfg['instances'])  # 1000
            end_ndx = 500
            start_ndx = self.cfg['instance'] * instance_size
        else:
            end_ndx = 1000
            start_ndx = self.cfg['instance']
...
class BASEV8Env_hssd(Env):
    ...
    def _initialize_experiment(self):
        ...
        """
        self.all_episodes = []
        self.sim_cfg['scene_config'] = "/file_system/vepfs/algorithm/dujun.nie/data/hssd-hab/hssd-hab-uncluttered.scene_dataset_config.json"
        self.goals = {}

        for f in sorted(os.listdir(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_hssd/{self.cfg["split"]}/content')):
            with gzip.open(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_hssd/{self.cfg["split"]}/content/{f}', 'rt') as gz:
                js = json.load(gz)
                hsh = f.split('.')[0]
                self.goals[hsh] = js['goals_by_category']
                self.all_episodes += js['episodes']
    ...
    def _initialize_episode(self, episode_ndx: int):
        ...
        self.sim_cfg['scene_path'] = f'/file_system/vepfs/algorithm/dujun.nie/data/hssd-hab/stages/{episode["scene_id"]}.glb'
        self.simWrapper = SimWrapper(self.sim_cfg)
