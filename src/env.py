...
class Env:
    ...
    def _calculate_metrics(self, agent_state: habitat_sim.AgentState, agent_action: PolarAction, geodesic_path: int, max_steps: int):
        ...
        # goal_position = np.array(self.current_episode['object_positions'])
        # metrics['distance_to_goal'] = np.min(np.linalg.norm(current_position - goal_position, axis=1))
        metrics['spl'] = 0
...
class GOATEnv(Env):
    ...
    def _initialize_experiment(self):
        ...
        self.sim_cfg['scene_config'] = "/file_system/vepfs/algorithm/dujun.nie/data/hm3d_v0.2/hm3d_annotated_basis.scene_dataset_config.json"

        self.all_episodes = []
        self.goals = {}
        for f in sorted(os.listdir(f'/file_system/vepfs/algorithm/dujun.nie/data/goat_bench/datasets/goat_bench/hm3d/v1/{self.cfg["split"]}/content')):
            with gzip.open(f'/file_system/vepfs/algorithm/dujun.nie/data/goat_bench/datasets/goat_bench/hm3d/v1/{self.cfg["split"]}/content/{f}', 'rt') as gz:
                js = json.load(gz)
                hsh = f.split('.')[0]
...
class ObjectNavEnv(Env):
    ...
    def _initialize_experiment(self):
        ...
        for f in sorted(os.listdir(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_hm3d_v2/{self.cfg["split"]}/content')):
            with gzip.open(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_hm3d_v2/{self.cfg["split"]}/content/{f}', 'rt') as gz:
                js = json.load(gz)
                hsh = f.split('.')[0]
    ...
    def _initialize_episode(self, episode_ndx: int):
        ...
        self.sim_cfg['scene_path'] = f'/file_system/vepfs/algorithm/dujun.nie/data/hm3d_v0.2/{self.cfg["split"]}/{f[1]}/{f[2]}'
        self.simWrapper = SimWrapper(self.sim_cfg)
...
class BASEV1Env(Env):
    """
    Environment for the BASEV1 task, extending the base Env class.
    This class defines the setup, initialization, and running of BASEV1 episodes.
    """
    ...
    def _step_env(self, obs: dict):
        ...
        goal_rotate, goal_flag, subtask = self.agent.make_plan(panoramic_image, desctiption, self.current_episode['object'])
        #  转向目标方向
        for j in range(min(11 - goal_rotate, 1 + goal_rotate)):
            if goal_rotate <= 6:
                obs = self.simWrapper.step(loop_action_clockwise)
...
class BASEV7Env(Env):
    ...
    def _step_env(self, obs: dict):
        ...
        panoramic_image, explorable_value, reason = self.agent.make_explorable_value(episode_images[-12:], self.current_episode['object'])
        goal_rotate, goal_reason = self.agent.update_evalue_value(explorable_value, reason)
        ...
        evalue_map = self.agent.draw_evalue_map(obs['agent_state'])
...
class BASEV8Env(Env):
    """
    Environment for the BASEV8 task, extending the base Env class.
    This class defines the setup, initialization, and running of BASEV2 episodes.
    """
    ...
    def _initialize_experiment(self):
        ...
        """
        self.all_episodes = []
        self.sim_cfg['scene_config'] = "/file_system/vepfs/algorithm/dujun.nie/data/hm3d_v0.1/hm3d_annotated_basis.scene_dataset_config.json"
        self.goals = {}

        for f in sorted(os.listdir(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_hm3d_v1/{self.cfg["split"]}/content')):
            with gzip.open(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_hm3d_v1/{self.cfg["split"]}/content/{f}', 'rt') as gz:
                js = json.load(gz)
                hsh = f.split('.')[0]
                self.goals[hsh] = js['goals_by_category']
                self.all_episodes += js['episodes']
    ...
    def _initialize_episode(self, episode_ndx: int):
        ...
        f = episode['scene_id'].split('/')[1:]
        self.sim_cfg['scene_id'] = f[1][2:5]
        self.sim_cfg['scene_path'] = f'/file_system/vepfs/algorithm/dujun.nie/data/hm3d_v0.1/{self.cfg["split"]}/{f[1]}/{f[2]}'
        self.simWrapper = SimWrapper(self.sim_cfg)
...
class BASEV8Env_mp3d(Env):
    ...
    def _initialize_experiment(self):
        ...
        """
        self.all_episodes = []
        self.sim_cfg['scene_config'] = "/file_system/vepfs/algorithm/dujun.nie/data/mp3d/mp3d_annotated_basis.scene_dataset_config.json"
        self.goals = {}

        for f in sorted(os.listdir(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_mp3d/{self.cfg["split"]}/content')):
            with gzip.open(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_mp3d/{self.cfg["split"]}/content/{f}', 'rt') as gz:
                js = json.load(gz)
                hsh = f.split('.')[0]
                self.goals[hsh] = js['goals_by_category']
                self.all_episodes += js['episodes']
    ...
    def _initialize_episode(self, episode_ndx: int):
        ...
        self.sim_cfg['scene_path'] = f'/file_system/vepfs/algorithm/dujun.nie/data/{episode["scene_id"]}'
        self.simWrapper = SimWrapper(self.sim_cfg)
...
class BASEV8Env_hssd(Env):
    ...
    def _initialize_experiment(self):
        ...
        """
        self.all_episodes = []
        self.sim_cfg['scene_config'] = "/file_system/vepfs/algorithm/dujun.nie/data/hssd-hab/hssd-hab.scene_dataset_config.json"
        self.goals = {}

        for f in sorted(os.listdir(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_hssd_0.2.3/{self.cfg["split"]}/content')):
            with gzip.open(f'/file_system/vepfs/algorithm/dujun.nie/data/objectnav_hssd_0.2.3/{self.cfg["split"]}/content/{f}', 'rt') as gz:
                js = json.load(gz)
                hsh = f.split('.')[0]
                self.goals[hsh] = js['goals_by_category']
                self.all_episodes += js['episodes']
    ...
    def _initialize_episode(self, episode_ndx: int):
        ...
        super()._initialize_episode(episode_ndx)
        episode = self.all_episodes[episode_ndx]
        self.sim_cfg['scene_id'] = episode['scene_id']
        self.sim_cfg['scene_path'] = f'/file_system/vepfs/algorithm/dujun.nie/data/hssd/stages/{episode["scene_id"]}.glb'
        #self.sim_cfg['scene_path'] = f'/file_system/vepfs/algorithm/dujun.nie/data/hssd/scenes/{episode["scene_id"]}.scene_instance.json'
        #self.sim_cfg['scene_path'] = episode['scene_id']
        self.simWrapper = SimWrapper(self.sim_cfg)

        goals = self.goals[episode['scene_id']]
        all_objects = goals[f'{episode["scene_id"]}_{episode["object_category"]}']
        view_positions = []
        for obj in all_objects:
            for vp in obj['view_points']:
                view_positions.append(vp['agent_state']['position'])
